...
echo '<form method="post" id="fotosave'.$uniq_id.'" action="?dir='.$_GET['dir'].'&fui='.$_GET['fui'].'&param='.str_replace('"',"%22",$_GET['param']).'&divid='.$_GET['divid'].'" enctype="multipart/form-data">';
foreach($fotos_info as $file=>$info)
{
	...
	if($ext=='swf'){
		echo '<div style="float:left;overflow:hidden;"><object wmode="opaque">';
		echo '<embed src="'.$folder.$file.'" width="150" wmode="opaque" quality="high"/>';
		echo '</object></div>';
	}
	else echo '<img src="'.$folder.$file.'?w=150" class="preview" /></div>';
...
echo sizeof($fotos_info)?'<input type="submit" value="'.$this->core_echomui('savebutton').'" onclick="return true;loadXMLDoc(\'/ajax-index.php?isadm=1&page=loadfotos&dir='.$_GET['dir'].'&fui='.$_GET['fui'].'&param='.str_replace('"',"\'",$_GET['param']).'&divid='.$_GET['divid'].'\',\''.$_GET['divid'].'\',\'fotosave'.$uniq_id.'\');">'."<input type=\"button\" value=\"".$this->core_echomui('go_sort_title')."\" onclick=\"if(confirm('".$this->core_echomui('proceedsort')."')){return showdialog('/".$this->adm_path."/sort_items/?".$params['dir_field_here']."=".$folder."&comid=".$params['com_id']."','".$this->core_echomui('go_sort_diti')."','','','','reloadfotolist".$_GET['fui']."');}\" />":$this->core_echomui('nopictures');
echo '</form>';
...
div.fotoitem{padding:10px 10px 15px 10px;border-top:#ccc 1px solid;overflow:hidden;}
div.fotoitem img.preview{float:left;margin:0px 10px 10px 0px;overflow:hidden;}
...
<?$this->jsonload[] = 'loadfotosonload()';?>
 <script>
    function loadfotosonload() {
        var h = Math.ceil(document.body.scrollHeight / 30) * 30;
        var ie = 0 /*@cc_on + @_jscript_version @*/;
        if (!ie) window.name = "h" + h;
        else {
			var hash = parent.location.hash;
			if(parent.location.hash) var newloc = parent.location.href.replace('#'+hash,"#h" + h);
			else var newloc = parent.location.href+"#h" + h;
