<?php //v.1.0.
if($this->go_addpage_cont=='y'){
$this->adm_get_com_config($com_id); // <- получили конфиг компонента из таблицы #__components
...
if($this->id)
{
	...
	$res = $this->query($sql);
	$page_row = $this->fetch_assoc($res);
}
$this->adm_get_editbookmarks($page_row); // <- получаем массив закладок с полями для редактирования
$hiddenstring = array(); // <- массив для вывода скрытых полей

$this->adm_showall4edit();
...
if(!$this->id && intval($this->adm_com_config['config']['defid'])) $this->id = intval($this->adm_com_config['config']['defid']);
$this->adm_showparttitle($this->core_echomui('admc_title')." / {edittext}"); // {edittext} автоматически заменяется на "редактировать" или "добавить" в зависимости от действия
