...
if($cur_params['tbl'])
{
	if($_POST['addfield']['field'])
	{
		...
		if(!$res) $this->adm_add_sys_mes($this->core_echomui('admc_fieldadderror').":<br><b>sql:</b> ".$sql."<br>", "err");
		else 
		{
	...
	if($_POST['editfield']['field'] && $_POST['editfield']['wasfield'])
	{
		...
		if(!$res) $this->adm_add_sys_mes($this->core_echomui('admc_fieldediterror').":<br><b>sql:</b> ".$sql."<br>", "err");
		else 
		{
...
?>
<?=$this->core_echomui('admc_tdmanage_table')?>: <b><?=$this->prefixed("#".$cur_params['tbl'])?></b><br><br>
