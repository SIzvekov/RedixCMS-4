		...
		switch($row['type'])
                               {
										...
										case 'fs_directory':
											...
											$return = "<span id='fs_directory_txt-".$uniq_id."'>".$selected_dir."</span><br/><a href='' onclick=\"loadfilemanager_".$uniq_id."();this.style.display='none';return false;\">".$this->core_echomui('fs_directory_choosefolder')."</a><span id='osnfldtxt-".$uniq_id."' style='display:none;'><a href='' onclick='setseldir_".$uniq_id."();return false;'>".$this->core_echomui('fs_directory_setthisdir')." <b><span id='selfldname-".$uniq_id."'></span></b></a></span><input type='hidden' name='".$row['db_fname']."' value='".$datas[$row['db_fname']]."' id='fs_directory_inp-".$uniq_id."'/><div id='fmgr-".$uniq_id."'></div>";
...
<?
        ...
        function adm_show_file($name="", $value="", $type="img", $fpathind1 = 0, $fpathind2 = 0,$style="",$extra="", $islink=1)
        {
                ...
                {
                        $echo .= "<small><font color='#cfcfcf'>".$this->core_echomui('adm_showfile_choosefile')."<b>".$this->core_ini_get("upload_max_filesize")."</b>:</font></small><br><input type='file' name='file_".$name."' ".$this->adm_inputclass($style)." ".$extra."><br>";
                ...
                if(!file_exists($dir1) || (file_exists($dir1) && !is_dir($dir1))) $echo .= "<font color='#ff0000'>".$this->core_echomui('adm_showfile_upnodir').": <b>".$dir1."</b></font><br>";
                if($thepath2) if(!file_exists($dir2) || (file_exists($dir2) && !is_dir($dir2))) $echo .= "<font color='#ff0000'>".$this->core_echomui('adm_showfile_upnodir').": <b>".$dir2."</b></font><br>";
	...
	// ^^^^^^^^^^^^^^^^^^^^^^^ EOF ADM_SHOW_LIDINPUT_ORIGINAL FUNCTION ^^^^^^^^^^^^^^^^^^^^^ простой input
        ...
        function adm_show_editor($name="",$text="",$id="",$h="",$w="",$class="",$extra="",$type="textarea",$wys_type="Default", $skin="default")
        {
                ...
                if($type && is_dir(DOCUMENT_ROOT."/editor/".$type) && file_exists(DOCUMENT_ROOT."/editor/".$type."/_run/_initial.php") && file_exists(DOCUMENT_ROOT."/editor/".$type."/_run/_config.php"))
                {
                        ...
                        include(DOCUMENT_ROOT."/editor/".$type."/_run/_config.php");
