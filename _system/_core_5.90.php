...
class core extends database {
	...
	return $response;
        ...
        function core_go_component()
        {
                ...
                if(!file_exists($compfile) || is_dir($compfile)) $this->core_fatal_error("Execute file of the required component <b>'".$this->component_name."'</b> does not exists");
...
//				if(intval($_SESSION['user']['group']['isadmin']) && $modul_info_row['editable']){$_TEXT = '<div style="border:1px solid #000">'.$_TEXT."</div>";}
        ...
        function core_show_page($TEXT)
        {
			if($_SESSION['user']['group']['isadmin']){
				...
				if(!$this->isadmin){
				$admpanel .= '<div class="panel">Вы авторизованы, как <b>'.$_SESSION['user']['login'].'</b> [<a href="http://'.HTTP_HOST.REQUEST_URI.(sizeof($_GET)?"&logoff":"?logoff").'">выйти</a>]. Перейти в <a href="/'.ADMINDIRNAME.'/'.$this->param.'/">панель администрирования</a>';
				...
				$admpanel .= '<div id="rxcmsstat" class="statwindow"><b>Статистика:</b><br/>
...
//				print_r($this);
        ...
        function core_debug($isdebug=0, $core_err=1, $db_err=1, $sql=1)
        {
                ...
                echo "<p>";
                if($core_err){
                        echo "<p><b>Core errors:</b><br>".join("<br>\n", $this->core_error)."</p>";
                }
                if($db_err){
                        echo "<p><b>DataBase errors:</b><br>".join("<br>\n", $this->db_errors)."</p>";
                }
                if($sql){
                        echo "<p><b>SQL queries have been executed:</b><br>".join("<br>\n", $this->db_sql)."</p>";
                }
                echo "</p>";
                return true;
        }
        ...
        function core_fatal_error($error)
        {
                ...
                die("<p><b>Fatal RedixCMS error:</b><br><span style='color: #ff0000'>".$error."</span></p>");
        }
